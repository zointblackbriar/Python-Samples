...
class BaseTestClass(unittest.TestCase):
    ...
    def test_levensthein_sentence_similarity_1(self):
        corenlpObject = StanfordSpacyNLP.TestConnectionCoreNLP()
        result = corenlpObject.similarity_levenshtein("Is the system status good for sensor1 in machine1?", "can you tell me the status for sensor1 in machine?")
        self.assertTrue(result)

    def test_jaro_winkler_sentence_similarity_1(self):
        corenlpObject = StanfordSpacyNLP.TestConnectionCoreNLP()
        result = corenlpObject.similarity_jaro_winkler("Is the system status good for sensor1 in machine1?", "can you tell me the status for sensor1 in machine?")
        self.assertTrue(result)

    def test_jaccard_sentence_similarity_1(self):
        corenlpObject = StanfordSpacyNLP.TestConnectionCoreNLP()
        result = corenlpObject.similarity_jaccard("Is the system status good for sensor1 in machine1?", "can you tell me the status for sensor1 in machine?")
        self.assertTrue(result)
